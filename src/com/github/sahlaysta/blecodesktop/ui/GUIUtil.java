...
final class GUIUtil {
	...
	private static Set<Component> addAllComponents(
			Container container, Set<Component> set) {
		set.add(container);
	...
	static void forEachComponent(
			Container container, Consumer<Component> c) {
		getAllComponents(container).forEach(c);
	}
	
	//Finds a Component from a Container based on predicate
	static <T extends Component> T getComponent(
			Container container, Predicate<T> p) {
