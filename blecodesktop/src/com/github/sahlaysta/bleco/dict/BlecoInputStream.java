...
final class BlecoInputStream extends DataInputStream {
	...
	//read entry
	Entry readEntry(ExampleSentence[] exampleSentences) throws IOException {
		//read strings
	...
	Entry[] readEntries(ExampleSentence[] exampleSentences) throws IOException {
	...
	Map<Byte, Entry[]> readChineseIndexMap(Entry[] entries) throws IOException {
		int size = readChar();
	...
	Map<String, Entry[]> readEnglishIndexMap(Entry[] entries) throws IOException {
		int size = readChar();
	...
	Map<String, Entry[]> readPinyinIndexMap(Entry[] entries) throws IOException {
		int size = readChar();
		Map<String, Entry[]> result = new HashMap<>(size);
		for (int i = 0; i < size; i++) {
			String key = readString();
