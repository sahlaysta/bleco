...
final class GUIShortcuts {
	...
	//define shortcuts used by the gui
	...
	private void setAcc(JMenuItem jmi, KeyStroke ks) {
		jmi.setAccelerator(ks);
		
		//nullify, these are handled by the searcher component
		if (jmi == gui.menuBar.next || jmi == gui.menuBar.previous ||
				jmi == gui.menuBar.open || jmi == gui.menuBar.close) {
			jmi.getInputMap(JMenuItem.WHEN_IN_FOCUSED_WINDOW).put(ks, "none");
			return;
		}
		
		//nullify the key action if the accelerator is banned
	...
	//get banned accelerators
	private KeyStroke[] getBannedAccelerators() {
		List<KeyStroke> list = new ArrayList<>();
		
		//get input map from root pane, has enter key presses
		...
		//get input map from toolbar, has arrow key presses
	...
	private void refreshMenuAccelerators() {
		setAcc(gui.menuBar.simplified, simplified[0]);
		setAcc(gui.menuBar.traditional, traditional[0]);
		setAcc(gui.menuBar.engSearch, forceEng[0]);
		setAcc(gui.menuBar.copySelEntry, copySelEntry[0]);
		setAcc(gui.menuBar.about, about[0]);
		setAcc(gui.menuBar.next, next[0]);
		setAcc(gui.menuBar.previous, prev[0]);
		setAcc(gui.menuBar.open, openEntry[0]);
		setAcc(gui.menuBar.close, exitEntry[0]);
	}
}